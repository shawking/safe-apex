...
!     DETERMINE TOTAL OUTPUT FROM A ROUTING REACH.
      ...
      END IF    
      WYLD(IDO)=10.*Z3*(QVOL(IDO)+RSSF(IDO)+SST(IDO)+QRF(IDO)+QDR(IDO)+CPVH(IDO))
      ...
      TNYL(IDO)=Z3*(QN(IDO)+RSFN(IDO)+QRFN(IDO)+QDRN(IDO))
      SP2=0.
      SP4=0.
      DO K=1,NDP
	      ...
	      SP4=SP4+YPST(K,IDO)
      END DO
      VARH(6,IDO)=Z3*YSD(NDRV,IDO)
	  SMMH(6,MO,IDO)=SMMH(6,MO,IDO)+VARH(6,IDO)
	  VARH(9,IDO)=Z3*(YN(IDO)+YNOU(IDO))
	  SMMH(9,MO,IDO)=SMMH(9,MO,IDO)+VARH(9,IDO)
	  VARH(11,IDO)=Z3*(YP(IDO)+YPOU(IDO))
	  SMMH(11,MO,IDO)=SMMH(11,MO,IDO)+VARH(11,IDO)
	  VARH(13,IDO)=Z3*(QN(IDO)+RSFN(IDO)+TSFN(IDO)+QRFN(IDO)+QDRN(IDO))
	  SMMH(13,MO,IDO)=SMMH(13,MO,IDO)+VARH(13,IDO)
	  VARH(19,IDO)=Z3*QP(IDO)
