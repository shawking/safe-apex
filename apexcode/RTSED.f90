...
!         DEP=Y1*(1.-EXP(-PRMT(81)*TRT))
   ...
   32 IF(RSMF<.1)GO TO 34
      DGU=MIN(.9*RSMF,.5*CVF(IDN2)*X5)*WSA2
...
!     DEP=DEP+(Y1-DEP)*(1.-EXP(-PRMT(81)*TRT))
      ...
      IF(X5>0.)THEN
          IF(RSMC<.1)GO TO 41
          DGU=MIN(.9*RSMC,.5*RCHC(IDN2)*X5)*WSA2
          RTON=WNOU(LD1,IDN2)/RSM1
          RTOP=WPOU(LD1,IDN2)/RSM1
          RTOC=WCOU(LD1,IDN2)/RSM1
      ELSE
   ...
   10 VAV=MAX(.1,(X1+VCH*QQ)/QAV)
	  ...
	  SDEP=SDEP+DEP*WSAX
	  SDEG=SDEG+DEG*WSAX
	  EVRT(IDN2)=MIN(Q2M3,10.*PRMT(91)*EO*WSU)
	  ...
	  WYLD(IDO)=Q2M3
      VAR(117,IDO)=Q2M3
      SMM(117,MO,IDO)=SMM(117,MO,IDO)+Q2M3
	  TNYL(IDO)=TNYL(IDN1)*QQ
      ...
      WNO3(LD1,IDN2)=WNO3(LD1,IDN2)+X2
      X2=QP(IDN1)*XX
      QP(IDO)=QP(IDN1)-X2
