...
!     THIS SUBPROGRAM SIMULATES ANIMAL GRAZING.
	  ...
	  END IF
	  ! DEMAND  
	  XX=GCOW(IHD,ISA)*GZRT(IHD,IOW)/(WSA(ISA)*HE(JT1))
      ...
      DO K=1,NN
          ...
          END IF
          CNLV(JJ)=MIN(BN(1,JJ),UN1(JJ,ISA)/(DM(JJ,ISA)*1000.))
      ...
      DO K=1,NN
          ...
          IF(YLD(JJ)>1.E-10)THEN
              ...
              ,TIL(JT1),CPNM(JD(ISA)),Y1,Y2,AGPM(ISA),STL(JJ,ISA),STD(JJ,ISA),&
              CNLV(JJ),BN(3,JJ)
          END IF 
      END DO 
      SMM(141,MO,ISA)=SMM(141,MO,ISA)+1.         
      RETURN
    3 FORMAT(1X,2I8,1X,6I4,1X,A8,1X,A4,5F10.2,6F10.4)
